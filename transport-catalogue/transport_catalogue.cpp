...
namespace transport_catalog
{
 ...
 void TransportCatalogue::AddBus(std::string name, std::vector<road_objects::Stop*> stops, bool is_roundtrip) {
    ...
    for (size_t i = 0; i < bus.stops.size() - 1; ++i) {
        ...
        route_length += GetDist(from->name, to->name);
...
void TransportCatalogue::SetDist(std::string from, std::string to, int dist_) {
        dist[{from,to}] = dist_;
}
    
int TransportCatalogue::GetDist(std::string from, std::string to) {
